...
namespace gracosmod123.items.botany.accessoriesbot.plant
{
    public class StarflowerWorld : ModWorld
    {
        ...
        {
            progress.Message = "Planting razor flower";

            for (int k = 0; k < (int)((double)(Main.maxTilesX * Main.maxTilesY) * 1E-06); k++)
            {
                GenerateOneStarflower(groundOnly: true);//wastrueIMPORTANT
            }
        }
        ...
        public override void PostUpdate()
        {
            ...
            for (int i = 0; (float)i < numTilesToUpdate; i++)
            {
                ...
                {
                    GenerateOneStarflower(groundOnly: false);
                }
            }
        }

        private void GenerateOneStarflower(bool groundOnly)
        {
            int yLimit;
            if (groundOnly)
            {
            ...
            int attempts = 0;
            int starflowerID = TileType<items.botany.accessoriesbot.plant.Starflowertile>();
            Tile tile;
            Tile tileBelow;
            ...
            while (!placeSuccessful)
            {
                ...
                if (!tile.active() && tileBelow.nactive() && !tileBelow.halfBrick() && tileBelow.slope() == 0 && tileBelow.type != TileID.PlanterBox && tileBelow.type != TileType<items.botany.accessoriesbot.plant.StarflowerPlanterBoxtile>())
                {
