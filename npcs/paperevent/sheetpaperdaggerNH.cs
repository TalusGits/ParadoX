...
namespace gracosmod123.npcs.paperevent
{
    public class sheetpaperdaggerNH : ModProjectile
    {
        ...
        {
            if (Main.rand.Next(2) == 0) Projectile.NewProjectile(projectile.Center.X, projectile.Center.Y - (float)Main.rand.Next(223, 265), 0.0f, 9.6f, mod.ProjectileType("paperdaggerNH"), projectile.damage, 7.0f);
            else Projectile.NewProjectile(projectile.Center.X, projectile.Center.Y + (float)Main.rand.Next(223, 265), 0.0f, -9.6f, mod.ProjectileType("paperdaggerNH"), projectile.damage, 7.0f);
        }
    }
}
