...
namespace gracosmod123.NPCs.paperevent.boss
{
    public class sheetpaperdagger : ModProjectile
    {
        ...
        {
            if (Main.rand.Next(2) == 0) Projectile.NewProjectileDirect(projectile.Center.X, projectile.Center.Y - (float)Main.rand.Next(223, 265), 0.0f, 9.6f, ModContent.ProjectileType("paperdagger"), projectile.damage, 7.0f);
            else Projectile.NewProjectileDirect(projectile.Center.X, projectile.Center.Y + (float)Main.rand.Next(223, 265), 0.0f, -9.6f, ModContent.ProjectileType("paperdagger"), projectile.damage, 7.0f);
        }
    }
}
